...
%  corrosion reaction                                                     %  
%  This is a version for using colloids instead of an iron film and is 
%  not complete as of 12/18/2020
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [dyeTracker, turnOnLocs, diffusionMovie, diffusionFig, onLocs, dyeProps,  labelNumber] = DiffusionColloid(startTime, T, dT, px, pxsize, Conc, D, sensMax, sensSigma, CA, cathLoc, frameRate, dye, ironProb, diffusionMovie, diffusionFig, linked)
%% set dye parameters
...
%    dyeProps = [];
%end
[dyeProps, ~] = MoveDyesNormal([], dimA, HitRate, UncerHR, px, pxsize, StepSize, Conc, 0); %create initial dye positions
...
for t=1:T
    ...
    while l <= size(dyeProps,1)
        ...
        if strcmp(dye, 'Resazurin') == 1 && CA(round(dyeProps(l,1)), round(dyeProps(l,2))) ~=0 && CA(round(dyeProps(l,1)), round(dyeProps(l,2))) ~=0.5 %if using resazurin which turns on at cathode and the dye is at a colloid surface
    ...
    [dyeProps, labelNumber] = MoveDyesNormal(dyeProps, dimA,HitRate,UncerHR,px, pxsize, StepSize, Conc, PerOn);
end
